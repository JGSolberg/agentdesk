...
def execute_local_run(db: Session, run_id: str) -> object:
    ...
    if not workspace or workspace.status.value != "active":
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Run workspace is not active")

    agent_service.update_run(db, run.id, agent_service.AgentRunUpdate(status=RunStatus.RUNNING, error=None))
    agent_service.append_log(db, run.id, agent_service.AgentRunLogAppend(level="info", message=f"Executing {agent.name} in {workspace.path}"))
    ...
    try:
        ...
        agent_service.append_log(db, run.id, agent_service.AgentRunLogAppend(level="error", message=message))
        return agent_service.update_run(db, run.id, agent_service.AgentRunUpdate(status=RunStatus.FAILED, error=message, result=(exc.stdout or "")))
    except OSError as exc:
        message = f"Unable to start agent command: {exc}"
        agent_service.append_log(db, run.id, agent_service.AgentRunLogAppend(level="error", message=message))
        return agent_service.update_run(db, run.id, agent_service.AgentRunUpdate(status=RunStatus.FAILED, error=message))

    if completed.stdout:
        agent_service.append_log(db, run.id, agent_service.AgentRunLogAppend(level="stdout", message=completed.stdout.rstrip()))
    if completed.stderr:
        agent_service.append_log(db, run.id, agent_service.AgentRunLogAppend(level="stderr", message=completed.stderr.rstrip()))

    if completed.returncode == 0:
        return agent_service.update_run(
            db,
            run.id,
            agent_service.AgentRunUpdate(status=RunStatus.SUCCEEDED, result=completed.stdout.rstrip() or "Command completed successfully", error=None),
        )
    ...
    return agent_service.update_run(
        db,
        run.id,
        agent_service.AgentRunUpdate(status=RunStatus.FAILED, result=completed.stdout.rstrip() or None, error=message),
    )
