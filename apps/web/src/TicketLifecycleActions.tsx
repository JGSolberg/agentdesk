...
type Pane = "menu" | "relationships" | "workspace" | "lifecycle";
...
export default function TicketLifecycleActions({ ticket, onChanged, onEdit, repositories, activeWorkspaces, archivedWorkspaces, workspaceRepositoryId, workspaceBusy, onWorkspaceRepositoryChange, onCreateWorkspace, onRemoveWorkspace }: Props) {
  ...
  function toggle() {
    setOpen((value) => { if (value) setPane("menu"); return !value; });
    setError(null);
  }

  function edit() { setOpen(false); setPane("menu"); onEdit(); }
  const paneTitle = pane === "relationships" ? "Relationships" : pane === "workspace" ? "Workspace" : "Lifecycle";
                  ...
                  {pane === "workspace" && (
                    <div className="lifecycle-menu-content">
                      {activeWorkspaces.map((workspace) => {
                        ...
                        return <div className="workspace-action-item" key={workspace.id}><div><strong>{workspace.branch}</strong><span>{repository?.name ?? "Repository"}</span><code>{workspace.path}</code></div><button type="button" disabled={workspaceBusy} onClick={() => void onRemoveWorkspace(workspace)}>Remove worktree</button></div>;
                      })}
                      {activeWorkspaces.length === 0 && cloneReadyRepositories.length > 0 && <><p>{archivedWorkspaces.length ? "Recreate the ticket workspace on its existing branch, or create it in another managed repository." : "Create an AgentDesk-owned worktree for this ticket."}</p><div className="ticket-workspace-create"><select value={workspaceRepositoryId} onChange={(event) => onWorkspaceRepositoryChange(event.target.value)}>{cloneReadyRepositories.map((repository) => <option key={repository.id} value={repository.id}>{repository.name}</option>)}</select><button type="button" disabled={workspaceBusy || !workspaceRepositoryId} onClick={() => void onCreateWorkspace()}>{workspaceBusy ? "Creating…" : archivedWorkspaces.length ? "Reactivate workspace" : "Create workspace"}</button></div></>}
                      {activeWorkspaces.length === 0 && cloneReadyRepositories.length === 0 && repositories.length > 0 && <p>Clone a project repository before creating a workspace.</p>}
