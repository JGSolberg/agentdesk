...
export default function TicketDetail() {
  ...
  async function makeWorkspace() {
    ...
    setError(null);
    try {
      await createWorkspace(workspaceRepositoryId, { ticket_id: ticketId });
      await reload(ticketId);
    } catch (cause) {
            ...
            <main className="ticket-detail-main">
              ...
              <section className="detail-section ticket-workspaces">
                ...
                {activeWorkspaces.map((workspace) => {
                  ...
                  return (
                    <article className="ticket-workspace-card" key={workspace.id}>
                      <div><strong>{workspace.branch}</strong><span>{repository?.name ?? "Repository"}</span><code>{workspace.path}</code></div>
                      <button type="button" onClick={() => void archiveWorkspace(workspace)} disabled={workspaceBusy}>Remove worktree</button>
                    </article>
                  );
                })}
                {activeWorkspaces.length === 0 && cloneReadyRepositories.length > 0 && (
                  <div className="ticket-workspace-create">
                    <select value={workspaceRepositoryId} onChange={(event) => setWorkspaceRepositoryId(event.target.value)}>
                      {cloneReadyRepositories.map((repository) => <option key={repository.id} value={repository.id}>{repository.name}</option>)}
                    </select>
                    <button type="button" onClick={() => void makeWorkspace()} disabled={workspaceBusy || !workspaceRepositoryId}>{workspaceBusy ? "Creating…" : "Create workspace"}</button>
                  </div>
                )}
