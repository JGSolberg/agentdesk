...
import { getTicket, listTicketEvents, type Ticket, type TicketEvent } from "./api/tickets";
  ...
  return (
    <section className="page ticket-detail-page">
      ...
      <header className="ticket-detail-header">
        <div>
          <div className="ticket-detail-kicker"><span>{ticket.ticket_key}</span><span>{ticket.type}</span></div>
          <h1>{ticket.title}</h1>
        </div>
          ...
          {[...events].reverse().map((event) => (
            <article className="activity-event" key={event.id}>
              ...
              <div>
                ...
                <p>{event.actor} · {new Date(event.created_at).toLocaleString()}</p>
              </div>
            </article>
          ))}
        </div>
